...
4. **Site Key** → buka `src/pages/kontak.astro` (atribut `data-sitekey` pada `<div class="cf-turnstile">`). Site key saat ini: `0x4AAAAAADtJ0fYZwDPE9bDW`.
5. **Secret Key** → set manual via Script Properties: key=`TURNSTILE_SECRET`, value=`<secret dari Cloudflare>`. Atau jalankan function `setup()` sekali.
...
> - `@graph`   : `sha256-osnTEfQPbDXGskDyFc5BVB6x+a74SuhKyOoC9sz7aes=`
